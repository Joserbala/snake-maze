...
namespace SnakeMaze.Structures.Utils
{
    ...
    {
        public static string InOrden(BinaryTree<T> t, int level)
        {
            string datastring = "";
            if (t != null)
            {
                datastring += string.Format("{0}:", level);
                datastring += "Left(" + InOrden(t.Left, level + 1) + ")"
                            + "[" + t.Root + "]"
                            + "Right(" + InOrden(t.Right, level + 1) + ")";
            }
        ...
        public static string InOrdenHorizontal(BinaryTree<T> t, int level)
        {
            string datastring = "";
            ...
            {
                if (t.Left != null)
                    datastring = string.Concat(datastring, BinaryTreeUtils<T>.InOrdenHorizontal(t.Left, level + 1));
                ...
                if (t.Right != null)
                    datastring += BinaryTreeUtils<T>.InOrdenHorizontal(t.Right, level + 1);
            }
